...
class YouTubeStudioMonitor:
    ...
    def get_program_profile_path(self):
        """프로그램 전용 Chrome 프로필 경로 반환"""
        profile_dir = os.path.join(os.path.expanduser("~"), "YouTubeStudioMonitor", "chrome_profile")
    ...
    def connect_to_chrome(self):
        ...
        try:
            if not self.is_chrome_debug_running():
                ...
                print("✅ Chrome이 YouTube Studio와 함께 시작되었습니다.")
                print("⏳ YouTube Studio 로딩 대기 중... (20초)")
                time.sleep(20)
            ...
            if self.channel_id:
                ...
                print("⚠️ 채널 ID를 추출할 수 없습니다. 수동으로 채널 페이지로 이동해주세요.")
    ...
    def replay_captured_request(self):
        ...
        try:
            ...
            post_data = self.captured_request['postData']
            ...
            replicated_headers = original_headers.copy()
            
            # 쿠키 추출 및 설정
            cookies_found = False
            if 'Cookie' in replicated_headers:
                cookie_str = replicated_headers['Cookie']
                print(f"🍪 쿠키 발견: {len(cookie_str)} bytes")
                ...
                print(f"   → {len(cookies)}개 쿠키 설정 완료")
                print(f"   ✅ Cookie 헤더도 유지 (YouTube는 둘 다 필요)")
                
                # ❗ 중요: Cookie 헤더를 삭제하지 않음! 
                # YouTube API는 Cookie 헤더와 쿠키 객체 둘 다 확인함
            else:
                print("⚠️ Cookie 헤더가 없습니다!")
            
            if 'cookie' in replicated_headers:
                cookie_str = replicated_headers['cookie'] 
                print(f"🍪 소문자 cookie 헤더도 발견: {len(cookie_str)} bytes")
                cookies = {}
                for item in cookie_str.split(';'):
                    if '=' in item:
                        key, value = item.strip().split('=', 1)
            ...
            # 중요한 헤더들 확인 및 수정
            important_headers = [
                'authorization', 'x-goog-authuser', 'x-youtube-client-name', 
                'x-youtube-client-version', 'x-youtube-delegation-context',
                'origin', 'referer', 'content-type', 'user-agent'
            ]
            ...
            for header in important_headers:
                ...
                # 대소문자 구분 없이 헤더 찾기
                for k in replicated_headers:
                    if k.lower() == lower_header:
                        found_header = k
                        break
                
                if found_header:
                    value = str(replicated_headers[found_header])
                    print(f"   ✅ {header}: {value[:50]}{'...' if len(value) > 50 else ''}")
                    
                    # User-Agent가 DevTools인 것 같으면 실제 브라우저 UA로 교체
                    if lower_header == 'user-agent' and ('HeadlessChrome' in value or 'DevTools' in value):
                        browser_ua = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/136.0.0.0 Safari/537.36'
                        replicated_headers[found_header] = browser_ua
                        print(f"   🔄 User-Agent 교체됨: {browser_ua}")
                else:
                    print(f"   ❌ {header}: MISSING")
            
            # JSON 데이터를 보낼 때 Content-Type 확인
            if post_data:
                try:
                    json.loads(post_data)  # JSON 파싱 테스트
                    content_type_header = None
                    for k in replicated_headers:
                        if k.lower() == 'content-type':
                            content_type_header = k
                            break
                    
                    if content_type_header:
                        if 'application/json' not in replicated_headers[content_type_header]:
                            print(f"   🔄 Content-Type 확인: {replicated_headers[content_type_header]}")
                    else:
                        replicated_headers['Content-Type'] = 'application/json'
                        print(f"   ➕ Content-Type 헤더 추가: application/json")
                except json.JSONDecodeError:
                    pass
            
            # Content-Length는 requests가 자동으로 계산하므로 제거
            replicated_headers.pop('Content-Length', None)
            
            # CURL 명령어 생성 (디버깅용)
            curl_command = self.generate_curl_command(url, method, replicated_headers, post_data)
            
            # 디버깅 정보 저장
            debug_file, curl_file = self.save_debug_info(original_headers, replicated_headers, curl_command)
            print(f"🐛 디버깅 정보 저장: {debug_file}")
            print(f"📜 CURL 명령어 저장: {curl_file}")
            ...
            if method.upper() == 'POST':
                if post_data:
                    ...
                    try:
                        ...
                        print(f"📦 JSON 데이터 전송: {len(json.dumps(json_data))} bytes")
                        response = session.post(url, headers=replicated_headers, json=json_data, timeout=30)
                    except json.JSONDecodeError:
                        # JSON이 아니면 raw data로 전송
                        print(f"📦 Raw 데이터 전송: {len(post_data)} bytes")
                        response = session.post(url, headers=replicated_headers, data=post_data, timeout=30)
                else:
                    print("📦 POST 데이터 없이 전송")
                    response = session.post(url, headers=replicated_headers, timeout=30)
            else:
                response = session.get(url, headers=replicated_headers, timeout=30)
            
            print(f"📡 요청 전송 완료 - 상태코드: {response.status_code}")
            
            # 응답 헤더 분석
            print(f"📥 응답 헤더: {len(response.headers)}개")
            if 'set-cookie' in response.headers:
                print(f"🍪 서버에서 새 쿠키 설정: {response.headers['set-cookie'][:100]}...")
            
            if response.status_code == 200:
                print("✅ 비디오 목록 가져오기 성공!")
                
                try:
                    api_response = response.json()
                    self.parse_and_save_video_data(api_response)
                    return api_response
                except json.JSONDecodeError:
                ...
                if response.status_code == 401:
                    print("\n🔍 401 에러 분석:")
                    print("   가능한 원인들:")
                    print("   1. 쿠키나 인증 토큰이 만료됨")
                    print("   2. CSRF 토큰이나 특정 헤더가 누락됨")
                    print("   3. Referer나 Origin 헤더 문제")
                    print("   4. 사용자 에이전트 불일치")
                    
                    if not cookies_found:
                        print("   ❗ 특히 쿠키가 없는 것이 가장 큰 문제로 보입니다!")
    ...
    def start_monitoring(self, duration=300):
        ...
        print(f"🔍 YouTube Studio 다중 탭 애널리틱스 수집 시작 ({duration}초 동안)")
        print("📋 수행할 작업:")
        ...
        if self.channel_id:
            print("🔄 자동으로 비디오 목록 페이지로 이동합니다...")
            self.navigate_to_videos_page()
        else:
            print("⚠️ 수동으로 YouTube Studio에서 비디오 목록 페이지로 이동해주세요!")
        ...
        # 지정된 시간 동안 대기
        print(f"⏰ {duration}초 동안 API 요청을 감지합니다...")
        ...
        try:
            ...
            for row_idx, item in enumerate(analytics_data, 2):
                ...
                data = [
                    ...
                    round(float(click_rate), 2) if click_rate else 0,
                ...
                for col_num, value in enumerate(data, 1):
                    ...
                    if col_num in [8, 9, 10, 11, 13, 14, 15]:  # 숫자 컬럼들 (조회수, 좋아요, 댓글, 노출수, 시청시간, 평균시청시간)
                        cell.alignment = Alignment(horizontal="right", vertical="center")
                        if isinstance(value, (int, float)) and value > 0:
                            cell.number_format = '#,##0'
                    elif col_num in [12, 16]:  # 퍼센트 컬럼들 (클릭률, 평균조회율)
                        cell.alignment = Alignment(horizontal="right", vertical="center")
                        if isinstance(value, (int, float)) and value > 0:
            ...
            # 행 높이 설정
            for row in range(1, len(analytics_data) + 2):
